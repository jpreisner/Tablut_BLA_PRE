...
public:
	...
	bool jouer(Terrain *terrain) {
		...
		std::cout << "Selectionnez les coordonnees (x,y) d'une case o� vous souhaitez deplacer le pion [" <<
		...
		while(!terrain->deplacerPion(x_avant, y_avant, x_apres, y_apres)){
			std::cout << "Selectionnez de nouvelles coordonnees (x,y) d'une case o� vous souhaitez deplacer le pion [" <<
					pionToString(terrain->get(x_avant, y_avant))<< "] (entrez x, puis entrez y) :" << std::endl;
			Utils::saisieSecureInt(x_apres);
			Utils::saisieSecureInt(y_apres);
		}
